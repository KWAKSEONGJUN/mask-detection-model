...
def face_detector(img, cascade, tc, filename):
    ...
    for x, y, w, h in results:
        ...
        trim_img = cv2.resize(trim_img, dsize=(IMG_SIZE, IMG_SIZE))
        cv2.imwrite(os.path.join(tc+filename), trim_img)


def img_process(sc, tc, cascade):
    files = os.listdir(sc)

    for filename in files[:101]:
